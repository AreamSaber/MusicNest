...
| 终端形态 | 鸿蒙 App（用户）+ Web 管理后台（员工/管理员）+ 统一后端服务 |
...
| 角色 | 标识 | 终端 | 能力概要 |
|------|------|------|----------|
| 普通用户 | `ROLE_USER` | 鸿蒙 App | 登录/实名、浏览乐器、下单支付、续租、归还预约、报修、评价 |
...
| `admin` | `ROLE_ADMIN` |
| `staff01` | `ROLE_STAFF` |
| 手机号 `13800001111` + 验证码 `123456` | C 端用户（已实名） |

> 员工账号首次密码可能为空，需通过 Web 设密页完成初始化。
...
本项目源码仅供学习与交流使用。商用或二次分发前，请自行评估依赖协议与业务合规要求。部署时请更换数据库口令、JWT secret 等敏感配置。
